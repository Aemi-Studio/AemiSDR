...
struct LiquidLensSnellTests {
    ...
    func uniformsAreFinite(material: LiquidLensMaterial) {
        ...
        #expect(indices.refractiveIndex.x.isFinite)
        #expect(indices.refractiveIndex.y.isFinite)
        #expect(indices.refractiveIndex.z.isFinite)
        ...
        #expect(indices.refractiveIndex.x >= 1.0)
        #expect(indices.refractiveIndex.y >= 1.0)
        #expect(indices.refractiveIndex.z >= 1.0)
    ...
    @Test func normalDispersionOrdering() {
        ...
        let uniforms = config.toUniforms(textureSize: SIMD2(200, 200), scale: 1)
        let indices = LiquidLensConfiguration.precomputedIndices(for: .water)
    ...
    /// Diamond's exact refractive index. The canonical Peter (1923) formula
    /// gives n ≈ 2.42 at 546 nm; this test pins the Sellmeier-derived value
    /// to that reference within the tolerance of the two-term fit.
    @Test func diamondHasCorrectRefractiveIndex() {
        let indices = LiquidLensConfiguration.precomputedIndices(for: .diamond)
        // Real diamond at 546 nm ≈ 2.430. Allow ±0.05 for the 2-term fit.
        #expect(abs(indices.refractiveIndex.y - 2.42) < 0.05)
    }

    /// Water's 4-term Daimon-Masumura fit is accurate to ~10⁻⁶ in the visible.
    /// Pins the Sellmeier-derived value near the published refractive index
    /// of water at 546 nm.
    @Test func waterHasCorrectRefractiveIndex() {
        let indices = LiquidLensConfiguration.precomputedIndices(for: .water)
        // Real water at 546 nm ≈ 1.3345.
        #expect(abs(indices.refractiveIndex.y - 1.3345) < 0.01)
    }
}
