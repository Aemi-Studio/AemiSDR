...
open class VisualEffectUIView: UIVisualEffectView {
  ...
  private let blurEffect = (NSClassFromString(_InternedKeys.customBlurEffectClass) as! UIBlurEffect.Type).init()
...
private extension VisualEffectUIView {
  ...
  func blurEffectValue<T>(forKey key: BlurEffectKey) -> T? {
    blurEffect.value(forKeyPath: key.rawValue) as? T
  }

  func setBlurEffectValue(_ value: (some Any)?, forKey key: BlurEffectKey) {
